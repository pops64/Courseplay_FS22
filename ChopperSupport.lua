...
function ChopperSupport:registerXmlSchema()
...
-- function ChopperSupport:loadUserSettings()
...
-- function ChopperSupport:saveUserSettings()
...
-- function ChopperSupport:showUserInformation(xmlFile, key)
...
function ChopperSupport:loadMap(filename)
	...
	self = FS22_Courseplay and FS22_AutoDrive.Courseplay
	-- CpUtil.info("Auto drive found: %s", tostring(self.autoDrive~=nil))

	-- g_courseEditor:load()
end

-- function ChopperSupport:deleteMap()
...
-- function ChopperSupport:setupGui()
...
-- function ChopperSupport.drawHudMap(map)
...
-- function ChopperSupport:loadMapDataHelpLineManager(superFunc, ...)
...
-- function ChopperSupport.saveToXMLFile(missionInfo)
...
function ChopperSupport:update(dt)
	-- g_devHelper:update()
	-- g_bunkerSiloManager:update(dt)
end

function ChopperSupport:draw()
...
---@param isUp boolean
---@param button number
function ChopperSupport:mouseEvent(posX, posY, isDown, isUp, button)
	-- if not g_gui:getIsGuiVisible() then
	-- 	local vehicle = g_currentMission.controlledVehicle
...
---@param isDown boolean
function ChopperSupport:keyEvent(unicode, sym, modifier, isDown)
	-- g_devHelper:keyEvent(unicode, sym, modifier, isDown)
end

function ChopperSupport:load()
...
function ChopperSupport.register(typeManager)
...
TypeManager.finalizeTypes = Utils.prependedFunction(TypeManager.finalizeTypes, ChopperSupport.register)
